package com.hungdt.waterplant.database;
...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public static final String COLUMN_NUMBER_WORKER= "NUMBER_WORKER";
    ...
    public static final String SQL_CREATE_TABLE_USER_DATA = "CREATE TABLE " + TABLE_USER_DATA + "("
            ...
            + COLUMN_NUMBER_WORKER + " TEXT NOT NULL, "
            + COLUMN_GEMS + " TEXT NOT NULL " + ");";
    ...
    public void createUserData(String userName, String rmNoti,String eNoti,String nbPlant,String nbWorker,String nbGems) {
        SQLiteDatabase database = instance.getWritableDatabase();
        ...
        values.put(COLUMN_NUMBER_WORKER, nbWorker);
        values.put(COLUMN_GEMS, nbGems);
    ...
    public void setNumberWorker(String oldNb, String nbWorker) {
        SQLiteDatabase db = instance.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(COLUMN_NUMBER_WORKER, nbWorker);
        db.update(TABLE_USER_DATA, values, COLUMN_NUMBER_WORKER + "='" + oldNb + "'", null);
        db.close();
    }

    public void setNumberGem(String oldNb, String nbGem) {
        SQLiteDatabase db = instance.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(COLUMN_GEMS, nbGem);
        db.update(TABLE_USER_DATA, values, COLUMN_GEMS + "='" + oldNb + "'", null);
        db.close();
    }
    ...
    public String getGems() {
        ...
        if (cursor.moveToFirst()) {
            while (!cursor.isAfterLast()) {
                nbGems = cursor.getString(cursor.getColumnIndex(COLUMN_GEMS));
                cursor.moveToNext();
            }
        }
        cursor.close();
    ...
    public String getNumberWorker() {
        ...
        if (cursor.moveToFirst()) {
            while (!cursor.isAfterLast()) {
                nbWorker = cursor.getString(cursor.getColumnIndex(COLUMN_NUMBER_WORKER));
                cursor.moveToNext();
            }
        }
        cursor.close();
